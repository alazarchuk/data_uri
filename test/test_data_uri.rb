...
describe DataURI::Data do
  ...
  describe "parsing" do
    ...
    describe "a base64 encoded image/gif data URI" do
      ...
      before do
        ...
        @uri = DataURI::Data.new("data:image/gif;base64,#{@base64}")
      end
  
      it "should parse as a DataURI::Data object" do
        _(@uri.class).must_equal DataURI::Data
      end
    ...
    describe "a text/plain data URI" do
      ...
      before do
        @uri = DataURI::Data.new("data:,A%20brief%20note")
      end
    ...
    describe "a text/html data URI with a charset" do
      ...
      before do
        @uri = DataURI::Data.new("data:text/html;charset=utf-8,%3C%21DOCTYPE%20html%3E%0D%0A%3Chtml%20lang%3D%22en%22%3E%0D%0A%3Chead%3E%3Ctitle%3EEmbedded%20Window%3C%2Ftitle%3E%3C%2Fhead%3E%0D%0A%3Cbody%3E%3Ch1%3E42%3C%2Fh1%3E%3C%2Fbody%3E%0A%3C%2Fhtml%3E%0A%0D%0A")
      end
    ...
    describe "a big data binary data URI" do
      ...
      it "should be parsed by DataURI::Data.new" do
        uri = DataURI::Data.new(@raw)
    ...
    describe "an invalid data URI" do
      it "should raise an error" do
        _{ DataURI::Data.new("This is not a data URI") }.must_raise(URI::InvalidURIError)
        _{ DataURI::Data.new("data:Neither this") }.must_raise(URI::InvalidURIError)
      end
    end
  ...
  describe "building" do
    ...
    it "given data and an explicit content_type" do
      uri = DataURI::Data.build(:content_type => 'image/gif', :data => StringIO.new(@data))
    ...
    it "given data with an implicit content_type" do
      ...
      uri = DataURI::Data.build(:data => io)
      _(uri.to_s).must_equal 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='
    end
    ...
    it "given data and no content_type" do
      ...
      uri = DataURI::Data.build(:data => io)
      _(uri.to_s).must_equal 'data:text/plain;base64,Zm9vYmFy'
    end
